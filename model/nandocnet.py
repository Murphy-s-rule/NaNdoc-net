...
class FeatureExtract(tf.keras.Model):
    def __init__(self,feature_extractor,d_model):
        super(FeatureExtract, self).__init__()
        #EfficientNet5 -> (batch size, 380, 380)
        self.feature_extract = feature_extractor(include_top=False,
                                                weights='imagenet')
        self.max_pooling = layers.MaxPooling2D((2, 2))
        self.reshape = layers.Reshape((25,2048))
        self.dense = layers.Dense(d_model, activation='relu')

    def call(self,img):
        batch_size = tf.shape(img)[0]
        x = efficientnet.preprocess_input(img)
        # EfficientNet5 -> (batch size, 380, 380, 3) -> (batch size, 10, 10, 2048)
        x = self.feature_extract(x,training=False)
        # pooling -> (batch size, 10, 10, 3) -> (batch size, 5, 5, 2048)
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask):
        ...
        scaled_attention, attention_weights = scaled_dot_product_attention(
            q, k, v, mask)
        nh = tf.shape(scaled_attention)[1]
        sl = tf.shape(scaled_attention)[2]
        dep = tf.shape(scaled_attention)[3]

        scaled_attention = tf.reshape(scaled_attention, (batch_size, nh, sl, dep))
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,
        ...
        # (batch_size, 50) -> (batch_size, 50, 128)
